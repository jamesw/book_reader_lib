...
import epub.ResourceLoader;
import epub.ResourceLoader.ResourceCallback;
...
public class BookView extends ScrollView {
	...
	public BookView(Context context, AttributeSet attributes) {
		...
		childView.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT,
	...
	public void setFileName(String fileName) {
		...
		this.loader = new ResourceLoader(fileName, context);
	}
...
//			flatten(ref.getChildren(), entries, level + 1);
	...
	private class ImageTagHandler extends TagNodeHandler {
		...
		public void handleTagNode(TagNode node, SpannableStringBuilder builder,
			...
			loader.registerCallback(spine.resolveHref(src), new ImageCallback(
...
/**		MediaType[] lazyTypes = {
				...
				MediatypeService.TTF, // We don't support custom fonts either
        ...
        InputStream bookStream = assetManager.open(fileName);
        Book newBook = epubReader.readEpub(bookStream);
