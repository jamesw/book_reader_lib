 ...
 */
package uk.co.fluffypaws.mobile_apps.page_turner_reader.view;

import epub.PageTurnerSpine;
...
public class SinglePageStrategy implements PageChangeStrategy {
	...
	public boolean isAtEnd() {
		return getPosition() + childView.getText().length() >= this.text
				.length();
	}
	...
	private int findStartOfPage(int endOfPageOffset) {
		...
		if (layout.getHeight() < bookView.getHeight()) {
			...
			int topLine = layout
					.getLineForVertical(layout.getHeight()
							- (bookView.getHeight() - 2 * bookView
									.getVerticalMargin()));
	...
	public void updatePosition() {
		...
		this.storedPosition = Math.min(this.text.length() - 1,
				this.storedPosition);
		...
		int bottomLine = layout.getLineForVertical(bookView.getHeight()
				- (2 * bookView.getVerticalMargin()));
		...
		if (layout.getHeight() >= bookView.getHeight() && text.length() > 10) {
			...
			/*
			 * Special case, happens with big pictures We increase the length of
			 * the text we display until it becomes to big for the screen, then
			 * cut off 1 before that.
			 */
			if (section.length() == 0) {
				for (int i = 1; i < cutOff.length(); i++) {
					section = cutOff.subSequence(0, i);
					layout = new StaticLayout(section, textPaint, boundedWidth,
							Alignment.ALIGN_NORMAL, 1.0f,
							bookView.getLineSpacing(), false);
					if (layout.getHeight() >= bookView.getHeight()) {
